import MSCDT.archs.common as common
import torch
...
def normalize(image):
    """
    Normalize a given image array to the range [0, 255].
    
    Parameters:
        image (np.ndarray): Input image array.
    
    Returns:
        np.ndarray: Normalized image array scaled to [0, 255].
    """
    image_min = image.min()
    image_max = image.max()
...
save_path = '/home/b109/XFH/MSCDT-master/MSCDT_1/results/result/IPRS1'

@ARCH_REGISTRY.register()
class MSCDTS1(nn.Module):
    def __init__(self, 
        n_encoder_res=6,         
        inp_channels=3, 
        out_channels=3, 
        ...
        super(MSCDTS1, self).__init__()
        
        self.save_IPRS1 = save_path
        ...
        os.makedirs(self.save_IPRS1, exist_ok=True)
    ...
    def forward(self, x, gt):
        if self.training:
            ...
            sr = self.G(x, IPRS1)
            '''
            iprs1_np = IPRS1.detach().cpu().numpy() 
            mat_save_path = os.path.join(self.save_IPRS1, f'IPRS1_train.mat')
            sio.savemat(mat_save_path, {'/home/b109/XFH/MSCDT-master/MSCDT_1/results/result/IPRS1': iprs1_np})
            iprs1_img = (iprs1_np - iprs1_np.min()) / (iprs1_np.max() - iprs1_np.min()) * 255
            iprs1_img = iprs1_img.astype(np.uint8) 
            Image.fromarray(iprs1_img).save(os.path.join(self.save_IPRS1, f'IPRS1_train.png'))
            '''
            return sr, S1_IPR
        else:
            IPRS1, _ = self.E(x,gt)
