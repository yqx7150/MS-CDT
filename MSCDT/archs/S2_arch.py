...
import MSCDT.archs.attention as attention
import torch
...
class denoise(nn.Module):
    def __init__(self,n_feats = 64, n_denoise_res = 5,timesteps=5):
        ...
        self.max_period=int(timesteps*10)
        n_featsx4=4*n_feats
        resmlp = [
...
save_path = '/home/b109/XFH/MSCDT-master/MSCDT_1/results/result/IPRS2_test'

@ARCH_REGISTRY.register()
class MSCDTS2(nn.Module):
    def __init__(self,         
        n_encoder_res=6,         
        inp_channels=3, 
        out_channels=3, 
        ...
        linear_end= 0.99, 
        timesteps = 4 ):
        super(MSCDTS2, self).__init__()
        
        self.save_IPRS2 = save_path
  ...
  linear_end= linear_end, timesteps = timesteps)
        ...
        os.makedirs(self.save_IPRS2, exist_ok=True)
    ...
    def forward(self, img, IPRS1=None):
        ...
        if self.training:
            ...
            iprs1_np = IPRS2.detach().cpu().numpy()
            mat_save_path = os.path.join(self.save_IPRS2, f'IPRS2.mat')
            sio.savemat(mat_save_path, {'/home/b109/XFH/MSCDT-master/MSCDT_1/results/result/IPRS2_test': iprs1_np})
            iprs1_img = (iprs1_np - iprs1_np.min()) / (iprs1_np.max() - iprs1_np.min()) * 255
            iprs1_img = iprs1_img.astype(np.uint8)
            Image.fromarray(iprs1_img).save(os.path.join(self.save_IPRS2, f'IPRS2.png'))
